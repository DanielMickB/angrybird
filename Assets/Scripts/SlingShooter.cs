...
public class SlingShooter : MonoBehaviour
{
    ...
    private Bird _bird;//daftar bird

    void OnMouseUp(){
        Collider.enabled = false;//mendisable collider setelah lepas klik , supaya tdk bisa narik ketapel lagi sampe reset
        Vector2 velocity = _startPos - (Vector2)transform.position;
        float distance = Vector2.Distance(_startPos, transform.position);
        
        _bird.Shoot(velocity, distance, _throwSpeed);//nembak tiap bird
        //Kembalikan ketapel ke posisi awal
        gameObject.transform.position = _startPos;
        Trajectory.enabled = false;//trajectory ilang setelah dilepas
    ...
    void OnMouseDrag(){
        ...
        if (!Trajectory.enabled){
            Trajectory.enabled = true;//nongolin trajectory
        }

        DisplayTrajectory(distance);
    }

    public void InitiateBird(Bird bird){//dari gamecontroller, ngebuat bird bisa di gerakin
        _bird = bird;
    ...
    void DisplayTrajectory(float distance){
        ...
        for (int i = 1; i < segmentCount; i++){//nyari posisi beberapa segmen selain 0
        ...
        for (int i = 0; i < segmentCount; i++){
            Trajectory.SetPosition(i, segments[i]);//ngegambar 
        }
    }
